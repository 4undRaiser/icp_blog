import {
  ...
  export function getPosts(): Result<Vec<Post>, string> {
    try {
      ...
      return Result.Err(`Failed to get posts: ${error}`);
    }
  }
  ...
  export function commentOnPost(
    ...
    if (!id) {
      return Result.Err<Post, string>("Invalid post id.");
    }
  
    if (!comment || comment.length > 1000) {
      return Result.Err<Post, string>("Invalid comment.");
    }
    ...
    try {
      ...
      return Result.Err<Post, string>(
        "An error occurred while commenting on the post."
      );
    }
  }
  ...
  export function updatePost(
    ...
    if (!payload.title || !payload.body || !payload.imageURL) {
      return Result.Err<Post, string>("invalid payload");
    }
    return match(postStorage.get(id), {
      Some: (post) => {
        if (post.owner.toString() === ic.caller().toString()) {
          return Result.Err<Post, string>(
            `requester is not the owner of the post with id=${id}`
          );
        }
        const updatedPost: Post = {
          ...post,
